...
class EMNIST():
    def __init__(self, directory="./data/", prefix="emnist-byclass-", num_classes=62):
        self._directory = directory
        self._num_classes = num_classes

        self._training_data = self._load_binaries(prefix + "train-images.idx3-ubyte")
        self._training_labels = self._load_binaries(prefix + "train-labels.idx1-ubyte")
        self._test_data = self._load_binaries(prefix + "test-images.idx3-ubyte")
        self._test_labels = self._load_binaries(prefix + "test-labels.idx1-ubyte")

        np.random.seed(0)
        samples_n = self._training_labels.shape[0]
        random_indices = np.random.choice(samples_n, samples_n // num_classes, replace=False)
        np.random.seed()
